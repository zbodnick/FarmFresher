<?php
  ...
  $approvedreturn = "SELECT COUNT(cid) approvedreturn FROM formone WHERE universityid = $uid AND cid = 'APPROVED';";
  ...
  $query3 = "SELECT SUM(chours) choursCSCI FROM transcript WHERE univerid = $uid AND crseid LIKE 'CSCI%';";
  ...
  {
    $queryMS = "UPDATE student SET applied_to_grad = 1 WHERE unid = $uid;";
  ...
  {
    $queryPHD = "UPDATE student SET applied_to_grad = 2 WHERE unid = $uid;";
  ...
  else
  {
    ...
    $queryGPA = "UPDATE student SET gpa = $avggpa WHERE unid = $uid;";
