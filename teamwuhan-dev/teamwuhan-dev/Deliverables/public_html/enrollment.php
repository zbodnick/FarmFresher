...
//select A.u_id, A.semester, A.year, A.grade, A.courseid from (select DISTINCT course.courseid, u_id, year, semester, grade, credits from courses_taken join schedule join course) as A;
  ...
  if ($result->num_rows > 0)
    ...
    if(strcmp($_SESSION['uType'], 'alumni') == 0){
      ...
      $query2 = "select sum(A.credits) from (select DISTINCT u_id, semester, year, grade, title, credits from courses_taken join schedule join catalog WHERE u_id = " . $id . " and catalog.c_id = courses_taken.crn) as A;";
